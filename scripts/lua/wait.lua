-- [MUSHCLIENT_API] 本文件包含以下MUSHclient内建API调用:

-- [MUSHCLIENT_API] ColourNote() - 彩色输出 (行174)

-- [MUSHCLIENT_API] GetPluginID() - 获取插件ID (行176)

-- [MUSHCLIENT_API] GetPluginInfo() - 获取插件信息 (行176)

-- [MUSHCLIENT_API] timer_flag - 定时器标志常量 (行98-102,135-139)
...
function wait.trigger_resume (name, line, wildcards, styles)
...
function wait.regexp (regexp, timeout, flags)
...
function wait.match (match, timeout, flags)

  return wait.regexp (MakeRegularExpression (match), timeout, flags)

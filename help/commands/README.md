...
| `/all /list` | 列出所有连接 |
| `/all /disconnect` | 断开所有连接（保留 session） |
| `/all /reconnect` | 断开并重新连接所有 session |
| `/all /sw` | ❌ **拒绝** |
| `/all /close` | ❌ **拒绝** |
| `/all /connect` | ❌ **拒绝** |
| `/all /profile` | ❌ **拒绝** |
...
**安全白名单**：允许 `/lua`、`/reload`、`/load`、`/list`、`/disconnect`、`/reconnect`。其它客户端命令（如 `/switch`、`/close`、`/profile`）在广播语境下无意义或危险，会被拒绝并提示允许的命令列表。
...
/all /reload                   所有角色重载脚本
/all /disconnect               断开所有连接
/all /reconnect                断开并重新连接所有角色
/all /list                     列出所有连接

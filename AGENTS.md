...
- 凭据去明文：四个凭据字段支持 `${ENV_VAR}` 占位符，启动自动加载 `.env`，变量缺失按空处理；全部落盘日志（命令/输出/lua/调试/panic）写入前对已登记凭据值统一脱敏（`***REDACTED***`），防登录密码明文落盘；未登记凭据时零拷贝快路径，高频输出不受影响
...
**定位**：服务于作者本人的 7×24 headless 挂机场景；私有脚本子模块（`scripts/private`）按设计不公开，非缺口（详见 ROADMAP §0）。

**Phase 1 — headless 运维闭环（当前最高优先级）**：
1. **守护进程模式** — ✅ 已实现（仅 Unix，`--daemon`；Windows 侧明确无此需求）
2. **远程控制 API** — HTTP/Unix Socket 接口，无人值守时外部查询状态、下发命令

**Phase 2 — 无人值守补全**：
3. **告警通知** — 断线/错误事件主动推送（Telegram/钉钉等）
4. **连接健康监控** — ✅ 核心已实现（v0.9.9 已强化重连可靠性，待 profile 启用心跳配置）
5. **凭据安全管理** — ✅ 已实现（`${ENV_VAR}` 占位符 + `.env` 自动加载，详见 README「凭据安全保存」）
6. **看门狗超时处理优化** — 补全超时现场信息（现有堆栈采集无效）+ 防重启风暴；abort 策略保留（已核实 exit() 有 flush 死锁风险）

**可选项（非缺口，有余力再做）**：扩展示例、文档完善。

**明确不做**（定位外，非缺口）：GMCP/MXP/MSDP 协议支持（目标服务器群不支持）、TLS 连接（均为明文 telnet）、GUI 与自动地图（与 headless 定位冲突）、面向陌生用户的产品化重构。

详见 `ROADMAP.md`（§0 为定位与技术边界）。

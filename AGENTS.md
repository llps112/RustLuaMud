...
**当前最高优先级（Phase 0 — 让优势可见）**：
1. **公开侧示例生态自足** — 补齐 example 到「一个角色从零跑通挂机」的完整演示
2. **上手文档与开箱配置** — 10 分钟入门、交互式 bootstrap、配置速查表

**Phase 1 — 可靠性收尾**：
3. **看门狗分级响应** — 超时前落盘现场、退出码区分、故障脚本隔离（当前 abort 会击杀全部会话）
4. **远程控制 API** — HTTP/Unix Socket 接口，可编程控制

**Phase 2 — 无人值守闭环**：
5. **告警通知** — 断线/错误事件主动推送（Telegram/钉钉等）
6. **守护进程模式** — ✅ 已实现（仅 Unix，`--daemon`；Windows 侧明确无此需求）
7. **连接健康监控** — ✅ 核心已实现（心跳检测、指数退避重连）
8. **凭据安全管理** — ✅ 已实现（`${ENV_VAR}` 占位符 + `.env` 自动加载，详见 README「凭据安全保存」）

**明确不做**（定位外，非缺口）：GMCP/MXP/MSDP 协议支持（目标服务器群不支持）、TLS 连接（均为明文 telnet）、GUI 与自动地图（与 headless 定位冲突）。

详见 `ROADMAP.md`（§0 为定位与竞品结论）。

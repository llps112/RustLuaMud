...
- 关闭连接回收按名登记（L8）：新增 `Logger::forget_session`，`/close` 时仅当无其它同名 session 存活才清除该 session 的凭据/保留数/清理游标登记，防 7×24 长跑频繁 `/profile load`/`/close` 下映射只增不减
...
- 新增 `.git-blame-ignore-revs` 登记纯移动提交；跨文件拆分场景该机制不生效，需配合 `git blame -C` 使用（文件内已注明边界）
- 同步文档与规则中 `src/lua/api.rs` 的失效路径引用

### v1.0.1 (2026-09-17)
- 日志热路径优化：目录轮转清理由「每条日志一次」收敛为「跨小时一次」，并去掉每条写入的正则编译与目录扫描（单条写入 115.53µs → 3.62µs；@324 条/秒时单核占用 3.7% → 0.12%）
- 触发器热路径优化：`styles` 表只在确有回调读取时构建（无匹配的行不再产出纯 GC 垃圾），matches 中从不使用的样式段克隆一并移除
- 新增回归测试钉住 `styles` 契约：匹配到 strip_ansi 后无可见字符的行（纯 SGR 行）时仍传 nil，与 `Simulate` 路径保持一致

### v1.0.0 (2026-09-17)
- 首个稳定版：Lua API、API 常量表、TOML 配置格式、CLI 参数、日志格式与退出信号自本版本起冻结，破坏性变更仅在 MAJOR 版本发生，弃用流程见 [COMPATIBILITY.md](COMPATIBILITY.md)
- `RUST_BACKTRACE` 改为用户优先的默认值：未设置时自动开启（panic 堆栈落盘），显式设置 `0` / `full` 一律尊重
- 看门狗超时日志记录可靠现场（回调名、超时阈值、实际耗时），移除只会采到看门狗线程自身睡眠栈的无效堆栈采集

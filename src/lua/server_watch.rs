...
//! 用于解决服务器间歇性无响应时指令堆积导致的限速惩罚问题。
//!
//! # 设计原理
//!
//! 本模块是针对典型的 MUD 指令限速机制设计的通用方案：
//! - 大多数 MUD 服务器会统计单位时间内的指令数，超过阈值后触发惩罚
//! - 当服务器无输出时，已发出的指令可能堆积在服务器排队，
//!   恢复后同时执行从而超过限速阈值
//!
//! 客户端自己的限速窗口（`cmd` 滑动窗口）控制向外发送的速率，
//! 本模块在此基础上增加一道被动安全门控：当服务器无输出超过 `pause_timeout` 时
//! 暂停发送，避免堆积超过 MUD 的惩罚线。
//!
//! # 参数设置说明
//!
...
//!
//! ```text
//! pause_timeout = (M - 余量) / R
//! ```
//!
//! 其中 `M` 是 MUD 的惩罚阈值（超过此值开始惩罚的指令数），
//! `余量` 是为网络波动预留的缓冲指令数。
//! 例如：客户端 18条/s，MUD 惩罚阈值 60，预留 15 条余量，
//! `pause_timeout = (60 - 15) / 18 ≈ 2.5s`。
//!
//! 其他参数也可按需调整，参见 `set_config` 方法。
